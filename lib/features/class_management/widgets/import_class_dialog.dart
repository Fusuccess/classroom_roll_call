...
class _ImportClassDialogState extends State<ImportClassDialog> {
  ...
  Future<void> _selectAndImportFile() async {
    try {
      ...
      // 选择文件
      final (filePath, fileBytes) = await ImportExportService.pickCSVFile();
      ...
      try {
        ...
        final (nameColumnIndex, idColumnIndex) =
            await ImportExportService.autoMatchColumns(
              filePath ?? '',
              fileBytes: fileBytes,
            );
        ...
        if (students.isEmpty) {
          setState(() {
            isLoading = false;
            errorMessage = '导入的 CSV 文件中没有有效的学生数据';
          });
          return;
        }
        ...
        setState(() {
          isLoading = false;
          selectedFilePath = filePath ?? 'CSV 文件';
        ...
        setState(() {
          isLoading = false;
          errorMessage = parseError.toString().replaceFirst('Exception: ', '');
        });
      }
    } catch (e) {
      if (!mounted) return;
      setState(() {
        isLoading = false;
        errorMessage = '文件选择失败: ${e.toString()}';
      });
    }
  }
